...
class GifFrame(tkinter.Frame):
    def __init__(self, parent_frame, mode):
        ...
        self.text = tkinter.Label(
            self, text="Do you agree with the model ?", font="{Comic Sans MS} 16"
        )
        ...
        self.gif_images = {
            "car": self._create_tk_image_list("car_gif", 48),
    ...
    def _create_tk_image_list(self, gif_name, image_number):
        ...
        gif_paths = (
            os.path.join(ROOT_PATH, "data", gif_name, frame) for frame in gif_frames
        )
    ...
    def button_agree(self):
        ...
        picture_name = (
            f"{self.parent_frame.predicted_class}_agree_{int(time.time())}.jpg"
        )
        os.rename(
            os.path.join(ROOT_PATH, "data", "picture", "last_capture.jpg"),
    ...
    def button_disagree(self):
        self.parent_frame.show_frame(f"{self.mode}_frame")
        picture_name = (
            f"{self.parent_frame.predicted_class}_disagree_{int(time.time())}.jpg"
        )
        os.rename(
            os.path.join(ROOT_PATH, "data", "picture", "last_capture.jpg"),
            os.path.join(ROOT_PATH, "data", "picture", picture_name),
    ...
    def animate(self):
        ...
        self.index = (self.index + 1) % len(
            self.gif_images[self.parent_frame.predicted_class]
        )
