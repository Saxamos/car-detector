...
class BaseFrame(ABC, tkinter.Frame):
    ...
    def _resize_and_convert_input_image(self, image):
        return image.resize((self.MODEL_HEIGHT, self.MODEL_WIDTH), Image.ANTIALIAS).convert('L')
    ...
    def _infer(self, image):
        ...
        logging.info('Probability not being a car: {}'.format(self.model.predict(image)[0, 0]))
    ...
    def __preprocess_input_image(self, image):
        ...
        reshaped_image = array_image.reshape((1, self.MODEL_HEIGHT, self.MODEL_WIDTH, 3))
        normalized_image = reshaped_image * 1 / 255
        return normalized_image

    @staticmethod
    def __interpret_model(image, model):
        plt.imshow(image[0])
        plt.show()
        ...
        model.summary()
        BaseFrame.__plot_activation(activations[0], 4, 8)
        BaseFrame.__plot_activation(activations[1], 4, 8)
        BaseFrame.__plot_activation(activations[2], 4, 5)
        BaseFrame.__plot_activation(activations[3], 4, 5)
        BaseFrame.__plot_activation(activations[4], 3, 4)
        BaseFrame.__plot_activation(activations[5], 3, 4)
        BaseFrame.__plot_activation(activations[6], 2, 4)
        BaseFrame.__plot_activation(activations[7], 2, 4)
        BaseFrame.__plot_neuron(activations[8], 20)  # 288)
        BaseFrame.__plot_neuron(activations[9], 10)  # 128)
        BaseFrame.__plot_neuron(activations[10], 1, vmin=0, vmax=1)
